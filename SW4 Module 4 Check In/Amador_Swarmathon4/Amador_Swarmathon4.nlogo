 ...
 extensions[bitmap]
  ...
  breed [spiral-robots spiral-robot]                  ;; Initialization of procedures for spiral-moving robots
  ;;DFS-robots breed
  breed [DFS-robots DFS-robot]                        ;; ;; Initialization of procedures for DFS-moving robots
...
to robot-control
  ...
  ;; Ask the turtles
  ask turtles [                                        ;; "Turtles" command which commands all agents to commence certain procedure
...
set stepCount maxStepCount
 ...
 ask neighbors[
   ...
   if pcolor = yellow[                ;; Re-written version of Process: Program which sets color tile(Rock) back into normal tile (floor) and changes the robots searching modes
 ...
 to return-to-base-spiral
   ...
   ifelse pxcor = 0 and pycor = 0[                   ;; If/Else chain made to identify the "base"/origin point and change the "robots" searching modes (and costume/shape) then send it back on its way
