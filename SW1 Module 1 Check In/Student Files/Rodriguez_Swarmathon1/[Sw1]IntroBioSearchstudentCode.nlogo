...
;;We need each robot to know some information about itself.
 ...
 to setup 
   ...
   create-robots numberOfRobots [  ;;numberOfRobots was inserted in order to adjust the amount of robots you wish to use.
     set shape "robot"
     set size 8
   ]
 ...
 to go
   ...
   [
     ...
     [
       ...
       ;;3) tell them what to do based on the answer  (PREVIOUS EXCERSISE) 
      ;;[look-for-rocks]
      ;;[return-to-base]
  ...
  if not any? patches with [pcolor = yellow][
    set numberOfRocks 0
  while [ numberOfRocks = 0][   ;;While the number of rocks is equal to zero,
     ask robots[                ;;the robot is asked
       if pcolor != green[      ;;if it is currently NOT on a green tile.
       return-to-base           ;;If it is not on a green tile, the robot intiates return-to-base command.
       set shape "robot"        ;;Robot's shape is returned to normal.
       set label ""             ;;Robot's "sf" label is cleared.
       forward 1                ;;Robot moves towards base.
       ]
      ]
     ]
    stop
 ...
 to look-for-rocks
   ...
   ;;   the one holding a rock.
       ...
       ask myself [
         ...
         if not usingSiteFidelity? [    ;;Turns on site fidelity if it's not already on.
           set usingSiteFidelity? true
           set label "sf"               ;;Put a label on robots that are using site fidelity.
           set siteX pxcor              ;;Store the x-coordinate
           set siteY pycor              ;;and y-coordinate of where we found this rock
           ]
         ]
        ]
       ]
 ...
 to do-site-fidelity
   ...
   ifelse(pxcor = siteX and pycor = siteY) ;;pxcor and pycor are the variables that describe the x and y coordinates of the patch the robot is currently standing on.
   [
     set usingSiteFidelity? false
     set label ""
     ]
