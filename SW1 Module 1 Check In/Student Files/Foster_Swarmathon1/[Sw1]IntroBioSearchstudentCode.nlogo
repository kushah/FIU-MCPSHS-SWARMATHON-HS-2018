...
;;We need each robot to know some information about itself.
 ...
 to go
   ...
   [
     ...
     ;;advance the clock
     tick
  ;;4) If there are no other rocks, return to base
  if not any? patches with [pcolor = yellow][  ;;if there are no yellow patches, set numberOfRocks 0
    set numberOfRocks 0
    ask robots
    [
      set searching? false 
      ifelse searching?     ;;if searching, look for rocks and when rocks run out, return to base
      [look-for-rocks]
      [return-to-base]
      while [pcolor != green]  ;;while patches are not green, continue to move forward
     [
 ...
 to do-site-fidelity
   ...
   ;;1)If the the robot is standing on a known x and y coordinate, then it is standing on a rock
   ifelse(pxcor = siteX and pycor = siteY)
   ;;2) Change the robot's mode to not usingSiteFidlelity? when robot arrvies back to known area where a rock once was, and turn of the label
   [ 
     set usingSiteFidelity? false
     set label ""
   ]
