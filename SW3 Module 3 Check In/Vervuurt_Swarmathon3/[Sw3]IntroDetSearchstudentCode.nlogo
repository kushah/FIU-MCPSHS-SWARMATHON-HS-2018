...
to make-robots
  create-robots numberOfRobots[
    set size 5          ;;set size for robot
    set shape "robot"   ;;set shape for robot
    set processingList? false  ;;set processingList false
    set returning? false ;;
    set rockLocations [] ;;set the rockLocations to zero
    set locX 0         ;;initial location for the x axis
    set locY 0         ;;initial location for the y axis
    set initialHeading random 360 ;;set initial heading to a random value between 0-360
    set heading initialheading  ;;set head
    if pen-down? [pen-down];; if pen-down is enabled set pen-down
  ]
