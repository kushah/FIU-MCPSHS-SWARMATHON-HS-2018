...
to make-robots
  create-robots numberOfRobots[      ;; Number of robots created are equal to the value of the numberOfRobots slider.
    set size 5                       ;; The size of the robot is set to 5.
    set shape "robot"                ;; The the robot image will be "robot" without the rock.
    set processingList? false        ;; The robots will not be working with the list of rock locations (processingList).
    set returning? false             ;; The robot is not currently returning to base (returning?).
    set rockLocations []             ;; The list of processingList on the robot is set up (rockLocations).
    set locX 0                       ;; The X location of the robot is set to 0.
    set locY 0                       ;; The Y location of the robot is set to 0.
    set initialHeading random 360    ;; The initial head is set to any degree between 0 and 360.
    set heading initialHeading       ;; The robot's intial heading is set to 360 degrees.
    if pen-down? [pen-down]          ;; If pen-down? is true turn on pen-down.
  ]
