...
;;Each robot knows some information about itself:
 ...
 to setup 
   ...
   while [targetLargeClusters > 0] [
     ask one-of patches[
       ...
       ]              ;;Makes sure the function block doesn't place a large rock cluster outside of the designated area or on top of a currently placed rock.
     ]
   ]
   set numberOfRocks numberOfRocks + (largeClusterRocks * 29)     ;; new total rock formula, which includes added large rock clusters
   ;;Make the base. 
   ask patches
   [
   ...
   ask neighbors[
     ...
     ;;   and change the patch color back to its original color.
      ...
      if count patches in-radius 1 with [pcolor = yellow] >= 2[
        ask myself [set usingPheromone? true]       ;;This if statement checks current surroundings of robot within a 1 patch radius for 2 or more rocks
     ]
   ]     
   ]
 ...
 ;; check-for-trails.
  ...
  if random 100 < percentChanceTofollowPheromone[
    ...
    set searching? false
    check-for-trails
  ]                                 ;;This block above sets probability for robots to follow pheromone trails
  ]                          
  ...
  [
    ...
    if usingPheromone? [
      ask patch-here[
        if pcolor != yellow [
          set pcolor cyan 
          set pheromoneCounter pheromoneDuration        ;; sets variable for pheromone trail duration time
        ]
      ]
   ]
  ]                         ;; Check in place to prevent pheromone trail from covering rocks
